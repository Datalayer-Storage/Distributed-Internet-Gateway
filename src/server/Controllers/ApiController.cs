...
[ApiController]
[Route("[controller]")]
public class ApiController(DatFileReaderService datFileReaderService) : ControllerBase
{
    private readonly DatFileReaderService datFileReader = datFileReaderService;
    [HttpGet]
    ...
    public IActionResult Upload([FromBody] UploadRequest request)
    {
        ...
        try
        {
            datFileReader.ReadDatFileToCache(request.FullTreeFilename);
